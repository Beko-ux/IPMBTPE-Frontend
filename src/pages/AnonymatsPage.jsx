...
export default function AnonymatsPage({ currentSection = "anonymats", onNavigate }) {
  ...
  // Liste des matières (avec isAnonymous)
  const [subjects, setSubjects] = useState([]);
  ...
  // Charger le contexte (evaluation_anonymats) et les sujets
  async function loadContext() {
    ...
    setMsg("");
    try {
      // 1) Charger les matières (avec isAnonymous)
      ...
      const subjectsData = await api.get(`/evaluations/subjects?${subjectsParams}`);
      ...
      const ctx = await api.get(`/evaluation-session-anonymats/context?${ctxParams}`);
      setExamCtx(ctx);
      ...
      // 3) Charger les anonymats existants
      await loadAnonymats(ctx.examId);
  ...
  async function loadAnonymats(examId) {
    const params = new URLSearchParams({ examId, includeStudents: includeStudents ? "1" : "0" });
  ...
  // Recharger anonymats si includeStudents change
  useEffect(() => {
    if (examCtx?.examId) loadAnonymats(examCtx.examId);
  }, [includeStudents]);
  ...
  async function generate() {
    ...
    if (examCtx.locked) return setErr("Anonymats verrouillés. Impossible de générer.");
    if (!examCtx.hasAnonymous) return setErr("Aucune ECUE en mode ANONYME (configure d'abord dans Évaluations).");
    if (!subjects.some(s => s.isAnonymous)) return setErr("Aucune ECUE en mode ANONYME (selon la liste ECUE).");
  ...
  async function saveManualAll() {
    ...
    setLoading(true);
    try {
      await api.post("/evaluation-session-anonymats/set", { examId: examCtx.examId, items });
      setMsg("Enregistré ✅");
      await loadAnonymats(examCtx.examId);
    } catch (e) {
      setErr(e.message);
  ...
  const printMeta = useMemo(() => ({
    ...
    semester,
    examType,
  }), [academicYear, selectedClass, semester, examType]);
            ...
            <div style={grid.twoCols}>
              ...
              <section style={card.base}>
                ...
                <div style={form.row}>
                  ...
                  <div style={{ ...form.field, justifyContent: "flex-end" }}>
                    <button style={btn.primary} onClick={loadContext} disabled={loading}>
                      Charger
                    </button>
                  </div>
                </div>
              ...
              <section style={card.base}>
                ...
                <div style={form.row}>
                  <div style={form.field}>
                    <label style={form.label}>Préfixe</label>
                    <input style={form.input} value={prefix} onChange={(e) => setPrefix(e.target.value)} placeholder="AT" disabled={examCtx?.locked} />
                    <div style={form.hint}>Ex: AT → AT101..AT120</div>
                  </div>
                  <div style={form.field}>
                    <label style={form.label}>StartAt</label>
                    <input style={form.input} value={String(startAt)} onChange={(e) => setStartAt(e.target.value)} placeholder="101" disabled={examCtx?.locked} />
                  </div>
                  <div style={form.field}>
                    <label style={form.label}>Width</label>
                    <input style={form.input} value={String(width)} onChange={(e) => setWidth(e.target.value)} placeholder="3" disabled={examCtx?.locked} />
                  </div>
                </div>
                ...
                <div style={form.row}>
                  <div style={{ ...form.field, justifyContent: "flex-end" }}>
                    <label style={form.label}>&nbsp;</label>
                    <label style={checkRow}>
                      <input type="checkbox" checked={force} onChange={(e) => setForce(e.target.checked)} disabled={examCtx?.locked} />
                      Forcer (réassigner tous)
                    </label>
                    <label style={{ ...checkRow, marginTop: 8 }}>
                      <input type="checkbox" checked={includeStudents} onChange={(e) => setIncludeStudents(e.target.checked)} />
                ...
                <div style={{ display: "flex", gap: 10, marginTop: 12, flexWrap: "wrap" }}>
                  ...
                  <button
                    style={btn.primary}
                    onClick={generate}
                    disabled={!examCtx?.examId || loading || !examCtx?.hasAnonymous || examCtx?.locked}
                  >
                    Générer anonymats
                  </button>
                  <button
                    style={btn.ghost}
                    onClick={saveManualAll}
                    disabled={!examCtx?.examId || loading || !anonymats.length || examCtx?.locked}
                  >
                    Enregistrer
                  </button>
                  <button
