...
export default function AnonymatsPage({ academicYear = "2025-2026", onNavigate }) {
  // Contexte général (synchronisé avec la prop academicYear)
  ...
  // Synchroniser year avec la prop academicYear si elle change
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    async function loadClasses() {
      try {
        const data = await api.get(`/classes?year=${encodeURIComponent(year)}`);
        if (!alive) return;
  ...
  const loadContext = useCallback(async () => {
    if (!year || !classId || !semester || !examType || !sessionName) {
  ...
  }, [year, classId, semester, examType, sessionName]);
  ...
  }), [year, selectedClass, semester, examType, subjects, selectedSubjectId]);

  return (
    <div style={{ maxWidth: "1600px", margin: "0 auto", padding: "0 18px 18px" }}>
      <style>{printStyle}</style>
      
      <h1 style={{ fontSize: "1.4rem", fontWeight: 700, marginBottom: "1.25rem" }}>Anonymats (attribution par matière)</h1>
...
// Styles (conservés mais adaptés pour le conteneur principal)
